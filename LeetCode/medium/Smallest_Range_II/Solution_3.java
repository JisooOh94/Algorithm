...
//        int[] arr = new int[]{0, 10}; int K = 2;
    ...
    public int smallestRangeII(int[] arr, int K) {
        ...
        int[] modifiedMin = new int[]{Math.min(minVal + K, maxVal - K), Math.min(minVal + K, maxVal - K)};
        int[] modifiedMax = new int[]{Math.max(minVal + K, maxVal - K), Math.max(minVal + K, maxVal - K)};

        for(int num : arr) {
            if(num - minVal < maxVal - num) num += K;
            else if(num - minVal > maxVal - num) num -= K;
            else {
                if (num + K < modifiedMin[0]) modifiedMin[0] = num + K;
                if (modifiedMax[0] < num + K) modifiedMax[0] = num + K;

                if (num - K < modifiedMin[1]) modifiedMin[1] = num - K;
                if (modifiedMax[1] < num - K) modifiedMax[1] = num - K;

                continue;
            }

            for(int i = 0; i < 2; i++) {
                if (num < modifiedMin[i]) modifiedMin[i] = num;
                if (modifiedMax[i] < num) modifiedMax[i] = num;
            }
        }
        return Math.min(modifiedMax[0] - modifiedMin[0], Math.min(modifiedMax[1] - modifiedMin[1],maxVal - minVal));
    }
}
