...
//        int[] arr = new int[]{1, 3, 6}; int K = 3;
    ...
    public int smallestRangeII(int[] arr, int K) {
        ...
        for(int num : arr) {
            ...
            int minusModifiedMin = modifiedMin;
            int minusModifiedMax = modifiedMax;
            if(minusModified < modifiedMin) {
                minusModifiedMin = minusModified;
            }
            if(modifiedMax < minusModified) {
                minusModifiedMax = minusModified;
            }

            int minusModifiedResult = minusModifiedMax - minusModifiedMin;

            int plusModifiedMin = modifiedMin;
            int plusModifiedMax = modifiedMax;
            if(plusModified < modifiedMin) {
                plusModifiedMin = plusModified;
            }
            if(modifiedMax < plusModified) {
                plusModifiedMax = plusModified;
            }

            int plusModifiedResult = plusModifiedMax - plusModifiedMin;

            if(minusModifiedResult <= plusModifiedResult) {
                modifiedMin = minusModifiedMin;
                modifiedMax = minusModifiedMax;
            } else {
                modifiedMin = plusModifiedMin;
                modifiedMax = plusModifiedMax;
            }
        }
