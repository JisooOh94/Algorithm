...
public class Solution {
	...
	private boolean recursion(int targetSum, int curSum, int leftSubSeq, int startIdx, boolean[] visited, int[] nums) {
		...
		for(int i = startIdx; i < nums.length; i++) {
			if(nums[i] + curSum > targetSum) break;
			if(visited[i]) continue;
			visited[i] = true;
			if(targetSum == curSum + nums[i] && recursion(targetSum, 0, leftSubSeq - 1, 0, visited, nums)) return true;
			else if(recursion(targetSum, curSum + nums[i], leftSubSeq, i + 1, visited, nums)) return true;
			visited[i] = false;
		}
		return false;
	}
