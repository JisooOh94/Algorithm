...
public class Solution {
	...
	private boolean isNotOverlapped(int idx_1, int idx_2) {
		return Math.abs(pos[idx_1][0] - pos[idx_2][0]) < 2 && Math.abs(pos[idx_1][1] - pos[idx_2][1]) < 2;
	}
	...
	private int recursion(int curIdx, int curCnt, boolean[] visited, int min, int max) {
		...
		for(int i = 0; i < 9; i++) {
			if(!visited[i] && isNotOverlapped(curIdx, i)) {
				visited[i] = true;
