...
//        int[] cost = new int[]{10, 15, 20};
    ...
    public int recursion(int idx, int[] cost, int[] record) {
        ...
        int cost_1 = idx + 1 < cost.length && record[idx + 1] != -1 ? record[idx + 1] : recursion(idx + 1, cost, record);
        int cost_2 = idx + 2 < cost.length && record[idx + 2] != -1 ? record[idx + 2] : recursion(idx + 2, cost, record);
